...
adding the parameter =--no-backup= or =-n= will force it to run without creating that file use =-v= for verbose output (or passing in options :verbose = false, :backup => false to Crontest.run)
